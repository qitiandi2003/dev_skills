...
def predict_nationality(surname, classifier, vectorizer):
    ...
    vectorized_surname = torch.tensor(vectorized_surname,
                                      dtype=torch.long,
                                      device=args.device).unsqueeze(0)  # 添加batch维度
    ...
    return {
        'nationality': vectorizer.nationality_vocab.lookup_index(indices.item()),
...
print("\n" + "="*50)
print(f"【最终测试结果】\n"
      f"测试集损失: {running_loss:.4f}\n"
      f"测试集准确率: {running_acc:.2f}%")
print("="*50)

# ====================== 指定姓氏预测 ====================== #
target_surnames = ["McMahan", "Nakamoto", "Wan", "Cho"]
print("\n" + "="*50)
print("指定姓氏预测结果:")
print("="*50)
...
for surname in target_surnames:
    try:
        ...
        print(f"{surname.ljust(10)} -> {prediction['nationality'].ljust(15)} "
              f"(概率: {prediction['probability']*100:.2f}%)")
    except KeyError as e:
        print(f"处理姓氏 '{surname}' 时出错: {str(e)}")